...
class no_bag_selector(nn.Module):
    """
    ...
    def __init__(self, config, hidden_size):
        super().__init__()
        self.config = config
        self.feature_to_classes = nn.Linear(hidden_size, config.num_classes)
        self.dropout = nn.Dropout(self.config.drop_prob)

    def forward(self, x):
        x = self.dropout(x)
        logits = self.feature_to_classes(x)
        return logits
    
    def test(self, x):
        x = self.dropout(x)
        x = self.feature_to_classes(x)
...
class Config():
    ...
    def test_one_epoch(self):
        ...
        for i in range(len(test_score)):
            for j in range(1, len(test_score[i])):
                test_result.append([j, self.data_test_label[i][j], test_score[i][j]])
        test_result = sorted(test_result, key=lambda x: x[2])
        test_result = test_result[::-1]
        pr_x = []
        pr_y = []
        each_pr = {}  # {"relation_id": [p, r, total]}
        correct = 0
        for i, item in enumerate(test_result):
            if item[0] not in each_pr.keys():
                each_pr[item[0]] = [0, 0]
            correct += item[2]
            each_pr[item[0]][0] += int(item[2])
            each_pr[item[0]][1] += 1
            pr_y.append(float(correct) / (i + 1))
            pr_x.append(float(correct) / self.total_recall)
        auc = sklearn.metrics.auc(x=pr_x, y=pr_y)
        print("auc: ", auc)
        return auc, pr_x, pr_y, each_pr
    ...
    def test(self):
        ...
        for epoch in self.epoch_range:
            ...
            auc, p, r, each_p= self.test_one_epoch()
            if auc > best_auc:
                best_auc = auc
                best_epoch = epoch
                best_p = p
        ...
        np.save(os.path.join(self.test_result_dir, self.model.__name__ + '_x.npy'), best_p)
        np.save(os.path.join(self.test_result_dir, self.model.__name__ + '_y.npy'), best_r)
        json.dump(open(os.path.join(self.test_result_dir, self.model.__name__ + '_each.json')), best_each_p)
        print("Finish storing")
        self.logger.info("Finish storing")

# model defination
class PCNNs(nn.Module):
    def __init__(self, config):
        super(PCNNs, self).__init__()
        self.config = config
        self.embedding = Embedding(config)
        self.encoder = PCNN(config)
        self.selector = no_bag_selector(config, config.hidden_size * 3)
...
def train_model():
    ...
    con.data_path = "/content/drive/My Drive/Colab Notebooks/BS/sem_120l_50w"
    con.max_epoch = 25
    con.max_length = 120
    con.load_train_data()
    con.load_test_data()
    con.set_train_model(PCNNs)
    con.train()
