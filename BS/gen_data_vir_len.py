...
def init(file_name, word_vec_file_name, rel2id_file_name, max_length = 120, case_sensitive = False, is_training = True):
    ...
    for i in range(len(ori_data)):
        ...
        sen_array = np.zeros(len(words))
        # sen_word
        for j, word in enumerate(words):
            # if j < max_length:
            if word in word2id:
                sen_array[j] = word2id[word]
            else:
                sen_array[j] = word2id['UNK']
        sen_word.append(sen_array)
    ...
    np.save(os.path.join(out_path, name_prefix + '_word.npy'), np.array(sen_word))
