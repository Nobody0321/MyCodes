...
class Embedding(nn.Module):
    def __init__(self, config):
        ...
        self.pos0_embedding = nn.Embedding(self.config.pos_range, self.config.pos_size, padding_idx = 0)
        self.pos1_embedding = nn.Embedding(self.config.pos_range, self.config.pos_size, padding_idx = 0)
        self.pos2_embedding = nn.Embedding(self.config.pos_range, self.config.pos_size, padding_idx = 0)
...
class Config():
    def __init__(self):
        ...
        self.max_length = 120
        self.pos_range = 2 * self.max_length
        self.batch_size = 160
        self.start_epoch = 0
        self.window_size = 3
            ...
            for batch in range(self.train_batches):
                ...
                print(
                    "epoch %d step %d time %s | loss: %f, penalty: %f, NA accuracy: %f, not NA accuracy: %f, total accuracy: %f\r" % (
                        epoch, batch, time_str, loss, penalty, self.acc_NA.get(), self.acc_not_NA.get(), self.acc_total.get()))
                self.logger.info(
                    "epoch %d step %d time %s | loss: %f, penalty: %f, NA accuracy: %f, not NA accuracy: %f, total accuracy: %f\r" % (
                        epoch, batch, time_str, loss, penalty, self.acc_NA.get(), self.acc_not_NA.get(), self.acc_total.get()))
...
class WordLevelSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.w_l1_s1 = nn.Linear(config.hidden_size * 2, config.d_att, bias=False)
        self.w_l1_s2 = nn.Linear(config.d_att, config.n_head, bias=False)
...
class MlssaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.lstm = nn.LSTM(input_size=config.input_dim, hidden_size=config.hidden_size, batch_first=True, bidirectional=True)
        self.att = WordLevelSelfAttention(config)
        self.w_l1_o = nn.Linear(config.n_head * config.hidden_size * 2, config.d_MLP_size)
...
class BagLevelAttention(nn.Module):
    def __init__(self, input_size, d_att, n_head):
        super().__init__()
        self.w_l2_s1 = nn.Linear(input_size, d_att, bias=False)
        self.w_l2_s2 = nn.Linear(d_att, n_head, bias=False)
...
def train_model():
    ...
    con.max_epoch = 15
    con.data_path = "/content/drive/My Drive/Colab Notebooks/BS/data-70len_200w2v"
    # con.data_path = "/content/drive/My Drive/Colab Notebooks/BS/mini_data"
    # con.data_path = "/content/drive/My Drive/Colab Notebooks/BS/data-120len_200w2v"
    # con.pretrain_model = "/content/drive/My Drive/Colab Notebooks/BS//checkpoint/Mlssa-1-0.28528476360085425"
    # con.start_epoch = 2
    con.load_train_data()
    con.load_test_data()
    con.set_train_model(Mlssa)
    con.train()
